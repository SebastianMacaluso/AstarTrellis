...
class IterJetTrellis(IterTrellis):
    ...
    def upperBoundOuters(self, elements: frozenset, proof=False):
        ...
        masses_sq = [leaves[k][0] ** 2 - np.linalg.norm(leaves[k][1::]) ** 2
             for k in range(len(leaves))
             ]
        # t_min = [tpi[pos, idx] if idx < (Nleaves - 1) else t_cut for pos, idx in enumerate(idxs)]
        t_min = np.sort([tpi[pos, idx] if idx < (Nleaves - 1) else t_cut for pos, idx in enumerate(idxs)])
    ...
    def upperBoundInners(self, elements: frozenset, proof=False):
        ...
        #      ], key=lambda x: x[0])
        #
        # E = [x for (x,y) in E_tupples]
        # E_indices = [y for (x,y) in masses_sq_tupples]
        ...
        #     values = np.concatenate((values, [ sum(masses_sq[0:i])] * (j // 2)))
        #     j = j // 2 + j % 2
        #     i += 1
        ...
        # mass_indices
        #
        # t_prev = 3 * (masses_sq[-(k - 1)] + masses_sq[-(k)]+2 *(leaves[mass_indices[-(k - 1)]][0] *leaves[mass_indices[-(k )]][0] + np.linalg.norm(leaves[-(k - 1)][1::]* np.linalg.norm(leaves[-(k)][1::]) )) + masses_sq[-(k + 1)]
        ...
        # llh = -np.log(1 - np.exp(- lam)) + np.log(lam) - np.log(values) - lam * values / subjetMass2
